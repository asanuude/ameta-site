...
const sectionEquipmentBuckets: Record<string, EquipmentBucket[]> = {
	...
	food: [
		...
		},
		{
			...
			priority: 2,
			patterns: [/(печь|плита|пароконвект|конвектомат|жароч|фритюр|гриль|мармит|котел|сковород|шкаф пекар|кипятиль|конфорк|тэн|термостат|нагрев|электросковород|вафельниц|блинниц|корн дог|хот дог|попкорн|сахарной ваты|рисоварк)/i],
		},
		{
			id: 'processing',
			name: 'Оборудование для приготовления и переработки',
			priority: 3,
			patterns: [/(мясоруб|овощерез|тестомес|миксер|куттер|слайсер|упаков|вакуум|фаршемеш|пила|блендер|измельч|протироч|планетарн|тестораскат|аппарат котлет|дозатор теста|дозатор жидкого теста|чипсов|электроглушения|лапшерезк|мешкозашивоч|запаив)/i],
		},
		{
			id: 'neutral',
		...
		},
		{
			...
			priority: 6,
			patterns: [/(ценникодерж|крючок|корзина покупател|стойка для распродаж|вешало|кассовый разделитель|стойка для сухофрукт|надставка для стойки|диспенсер|площадка гиревая|сейф|кассовый бокс|примерочн|гигрометр|рециркулятор)/i],
		},
		{
			id: 'utensils',
			name: 'Посуда и кухонный инвентарь',
			priority: 7,
			patterns: [/(посуда|тарелк|чашк|блюдц|ложк|вилк|нож |гастроемк|контейнер|поднос|миск|кастрюл|сотейн|лоток|противень|лопатк|насадка кондитер|моющее ср|ополаскивающ|решетка приемная)/i],
		},
		{
			id: 'parts',
			name: 'Запчасти и комплектующие',
			priority: 8,
			patterns: [/(тэн|термостат|датчик|реле|лампа|сальник|вентилятор|мотор|двигател|редуктор|подшипник|уплотн|прокладк|запчаст|ручк|переключател|выключател|комплектующ|втулк|полотно|провод|спирал|шатун|пульт управлен|колесо)/i],
		},
		{
			id: 'inventory',
