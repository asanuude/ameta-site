...
const sectionEquipmentBuckets: Record<string, EquipmentBucket[]> = {
	...
	food: [
		...
		},
		{
			...
			priority: 2,
			patterns: [/(печь|плита|пароконвект|конвектомат|жароч|фритюр|гриль|мармит|котел|сковород|шкаф пекар|кипятиль|конфорк|тэн|термостат|нагрев|электросковород)/i],
		},
		{
			id: 'processing',
			name: 'Оборудование для приготовления и переработки',
			priority: 3,
			patterns: [/(мясоруб|овощерез|тестомес|миксер|куттер|слайсер|упаков|вакуум|фаршемеш|пила|блендер|измельч|протироч|планетарн)/i],
		},
		{
			id: 'neutral',
		...
		},
		{
			...
			priority: 7,
			patterns: [/(тэн|термостат|датчик|реле|лампа|сальник|вентилятор|мотор|двигател|редуктор|подшипник|уплотн|прокладк|запчаст|ручк|переключател|выключател|комплектующ)/i],
		},
	],
	video: [
		{
